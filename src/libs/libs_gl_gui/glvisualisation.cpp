...
void GLVisualisation::init(std::string window_name, unsigned int window_width, unsigned int window_height)
{
  if (g_rl_visualisation_gl_init_done == false)
  {
    ...
    std::string textures_file_name = "textures.json";
    textures.load(textures_file_name);
...
unsigned int GLVisualisation::width()
{
  return window_width;
}

unsigned int GLVisualisation::height()
{
...
void GLVisualisation::paint_textured_rectangle(float width, float height, unsigned int texture_id)
{
  GLuint gl_texture = textures.get(texture_id);
...
GLuint GLVisualisation::get_texture(unsigned int id)
{
